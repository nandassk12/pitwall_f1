...
function RaceControlRow({ msg }) {
  ...
  const color = msg.flagColor || '#cbcbd6ff';
  ...
  return (
    <div style={{
      ...
      {/* Lap number */}
      <span style={{ color: '#e0e1e9ff', textAlign: 'center' }}>
        {msg.lap != null ? `L${msg.lap}` : '—'}
      </span>

      {/* Time */}
      <span style={{ color: '#e0e1e9ff', fontSize: '10px' }}>
        {formatTime(msg.date)}
      </span>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ color: '#e10600', fontWeight: 'bold', fontSize: '14px', letterSpacing: '1.5px' }}>
            ◆ RACE CONTROL
          </span>
          <span style={{
            fontSize: '14px',
      ...
      <div style={{
        ...
        {visible.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#a82222ff', padding: '30px', fontSize: '20px' }}>
      ...
      <div style={{
        ...
        {['Flag', 'SafetyCar', 'Drs'].map(cat => {
          ...
          return (
            <span key={cat}>
              {cat.toUpperCase()}: <span style={{ color: '#c8c8cfff' }}>{count}</span>
            </span>
          );
        })}
        <span style={{ marginLeft: 'auto' }}>
          SHOWING: <span style={{ color: '#bbbbc4ff' }}>{visible.length} / {messages.length}</span>
        </span>
      </div>
    </div>
  );
